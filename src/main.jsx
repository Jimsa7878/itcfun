    ...
    return (
      <main className="shell shell--game shell--host">
        ...
        <section className="teams-panel">
          ...
          {teams.length === 0 ? <p className="empty-state">Waiting for teams to join with the room code.</p> : rankedTeams.length === 0 ? <p className="empty-state">All teams have bingo.</p> : <div className="team-list">{rankedTeams.map(({ team, progress }, index) => { const progressSummary = [4, 3].map((length, summaryIndex) => { const count = progress.counts[3 - summaryIndex]; return count ? `${count} line${count === 1 ? '' : 's'} at ${length}/5` : ''; }).filter(Boolean).join(' · '); return <div className="team-row" key={team.id}><span className="team-rank">{index + 1}</span><span className="team-name">{team.name}</span><span className="team-progress">{progressSummary || `${progress.total}/25 marked`}</span></div>; })}</div>}
        </section>
      </main>
    );
  }
